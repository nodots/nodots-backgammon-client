...
import { Move, isMove, pointToPoint, reenter, MoveStatus } from '../move'
      ...
      if (isPoint(destinationPoint)) {
        if (
          ...
          (destinationPoint.checkers.length >= 1 &&
            destinationPoint.checkers.filter(
              (c) => c.color !== turn.player.color
            ).length === 0) ||
...
export function getNextPointPosition(
  position: number,
  dieValue: DieValue,
  moveDirection: MoveDirection
) {
  const nextPosition =
    moveDirection === 'clockwise' ? position + dieValue : position - dieValue
  return nextPosition
}
...
function getMoveDestination(
  ...
  const bearOffQuadrant = board.quadrants.find(
  ...
  if (board.bar[player.color].checkers.length > 0) {
    {
      const checkerToMove =
        board.bar[player.color].checkers[
          board.bar[player.color].checkers.length - 1
        ]
      // Where can we move? Back to our home quadrant.
      if (player !== undefined) {
        const homeQuadrant = board.quadrants.find(
          (q) => q.location === getHomeQuadrantLocation(player.moveDirection)
        )
      }
    }
  }
